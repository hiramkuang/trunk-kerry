...
class ChunkingApp(QMainWindow):
    ...
    def init_models(self):
        try:
            ...
            # 尝试加载主模型
            main_model_path = os.path.join(models_dir, Config.EMBEDDING_MODEL)
            logger.info(f"尝试加载主模型: {main_model_path}")
            
            if os.path.exists(main_model_path):
                self.embedding_model = CustomEmbeddingModel(main_model_path)
                logger.info(f"成功加载主模型: {main_model_path}")
                self.log_output.append(f"已加载模型: {Config.EMBEDDING_MODEL}")
                return
                
            # 尝试加载备用模型
            backup_model_path = os.path.join(models_dir, Config.BACKUP_EMBEDDING_MODEL)
            logger.info(f"尝试加载备用模型: {backup_model_path}")
            
            if os.path.exists(backup_model_path):
                self.embedding_model = CustomEmbeddingModel(backup_model_path)
                logger.info(f"成功加载备用模型: {backup_model_path}")
